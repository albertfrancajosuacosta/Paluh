...
class JanelaNormalidade(Toplevel):
    ...
    def criarJanela(self):
        ...
        h = Scrollbar(output_container, orient='horizontal')
        h.pack(side=BOTTOM, fill='x')

        self.st = ScrolledText(output_container)
        self.st.pack(fill=BOTH, expand=YES)
        h.config(command=self.st.xview)
    ...
    def testar(self):
        ...
        self.sig = self.util.converteNivelSignificancia(self.nivelSignificanciaeEscolhido.get(),
                                                        self.tipoTesteNormalidadeEscolhido.get())
        ...
        try:
            ...
            if self.qtdColunasPlanilha != 1:
                Messagebox.show_error(title="Erro",
                                      message="Os dados precisam estar em uma única coluna. Atualmente os dados estão em " + str(
                                          self.qtdColunasPlanilha) + ' colunas.')
            else:
    ...
    def shapiroWilk(self):
        ...
        self.textoResultado.insert("end",
                                   "Resultado - " + self.tipoTesteNormalidadeEscolhido.get() + " Nível de Significância " + str(
                                       te.nivelSignificancia) + "\n", "h1")
